...
class SongsIndex extends Component {
  ...
  componentDidMount() {
    axios
      .get(`/api/artists/${this.props.id}/songs`)
      .then(res => {
  ...
  addSong = song => {
    axios
      .post(`/api/artists/${this.props.id}/songs`, { song })
      .then(res => {
  ...
  updateSong = (id, song) => {
    axios
      .put(`/api/artists/${this.props.id}/songs/${id}`, { song })
      .then(res => {
  ...
  deleteSong = id => {
    axios.delete(`/api/artists/${this.props.id}/songs/${id}`).then(res => {
...
export class ConnectedSongs extends Component {
  render() {
    return (
      <AuthConsumer>
        {auth => <SongsIndex {...this.props} auth={auth} />}
      </AuthConsumer>
    );
  }
}
