...
class ArtistIndex extends Component {
  state = { artists: [], editing: false, addForm: false };
  ...
  componentDidMount() {
    ...
    axios
      .get(`/api/genres/${this.props.id}/artists`)
      .then(res => {
  ...
  addArtist = artist => {
    axios
      .post(`/api/genres/${this.props.id}/artists`, { artist })
      .then(res => {
  ...
  updateArtist = (id, artist) => {
    axios
      .put(`/api/genres/${this.props.id}/artists/${id}`, { artist })
      .then(res => {
        const artist = this.state.artists.map(artist => {
          if (artist.id === id) return res.data;
          return artist;
        });
  ...
  deleteArtist = id => {
    axios.delete(`/api/genres/${this.props.id}/artists/${id}`).then(res => {
  ...
  renderTabs = () => {
    let panes = this.state.artists.map(artist => {
      return {
        ...
        render: () => (
          <Tab.Pane style={{ backgroundColor: "transparent" }} attached={false}>
...
export class ConnectedArtists extends Component {
  render() {
    return (
      <AuthConsumer>
        {auth => <ArtistIndex {...this.props} auth={auth} />}
      </AuthConsumer>
    );
  }
}
